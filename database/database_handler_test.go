...
func TestNewDatabaseHandler_UnsupportedDriver(t *testing.T) {
	...
	cfg := DatabaseConfig{Driver: "sqlite", DSN: "file:test.db", Table: "logs", Level: slog.LevelInfo}
	_, err := NewDatabaseHandler(cfg)
	if err == nil {
...
func TestNewDatabaseHandler_EmptyDSN(t *testing.T) {
	cfg := DatabaseConfig{Driver: "mysql", DSN: "", Table: "logs"}
	_, err := NewDatabaseHandler(cfg)
	// Validate 应该返回错误，因为 DSN 不能为空
	if err == nil {
		t.Fatal("NewDatabaseHandler should reject empty DSN")
	}
}
