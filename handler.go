...
func buildHandler(opts ...HandlerOption) (slog.Handler, error) {
	...
	return safeMultiHandler(slog.NewMultiHandler(o.handlers...), o.handlers), nil
}

// safeMultiHandler 包装 slog.MultiHandler，每个子 Handler 调用时加 panic recover。
// 防止单个 Handler panic 导致整个日志调用崩溃。(Decision #17)
func safeMultiHandler(multi slog.Handler, handlers []slog.Handler) slog.Handler {
	return &safeMulti{
		multi:    multi,
		handlers: handlers,
	}
...
func (h *safeMulti) Handle(ctx context.Context, r slog.Record) error {
	if len(h.handlers) == 1 {
		...
		defer func() {
			if r := recover(); r != nil {
				fmt.Fprintf(os.Stderr, "mebsuta: handler panic recovered: %v\n", r)
			}
		}()
