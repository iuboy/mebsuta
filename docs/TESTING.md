...
| Audit JSON promotes reserved metadata fields | P0 | `TestContractJSONHandler_AuditFieldPromotion`, `audit.TestAuditEvent_ExplicitType` | covered | `event_type`, `actor`, `success` 为顶层字段 |
...
| Audit records pass through Error-level filtering | P0 | `TestWithSampling_AuditAlwaysRecorded`, `audit.TestLevelAudit_Value` | covered | SPEC 要求 Audit >= Error severity |
...
| Size-based rotation creates backups and a fresh active file | P1 | `filerotate.TestWriter_SizeRotation` | covered | 活跃文件权限必须保持受限 |
| Backup retention by count | P1 | `filerotate.TestWriter_MaxBackups` | covered | |
| Gzip compression of rotated logs | P1 | `filerotate.TestWriter_Compress`, `filerotate.TestCompressResidual` | covered | 使用临时文件后 rename |
...
| Sampling preserves error and audit records | P0 | `TestWithSampling_AuditAlwaysRecorded` | covered | SPEC 要求 |
...
| --- | --- | --- |
| mebsuta (main) | ~80% | 2026-05-24 |
| filerotate/ | ~85% | 2026-05-24 |
| audit/ | ~90% | 2026-05-24 |
| database/ | ~76% | 2026-05-24 |
| metrics/ | ~90% | 2026-05-24 |
